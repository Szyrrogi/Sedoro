...
# === USTAWIENIA NAGRÓD (KARTY) ===
@export_category("Card Rewards Settings")
@export var reward_cards_count: int = 1 # Ile kart gracz dostanie za wygranie pokoju
@export var reward_allowed_rarities: Array[int] = [0, 1] # Rzadkość: 0 = zwykłe, 1 = rzadkie itd.
@export var specific_reward_cards: Array[int] = [] # Jeśli podasz tu ID kart (np. [8, 13]), zignoruje losowanie i da te konkretne karty!
# =================================
...
func get_random_card_rewards() -> Array:
	...
	# Zmień na poprawną ścieżkę do bazy kart, jeśli masz inną!
	const CardDatabase = preload("res://Scripts/CardDatabase.gd") 
	
	# 1. Jeśli ustawiłeś konkretne karty "z palca" w Inspektorze, zwróć je od razu
	if specific_reward_cards.size() > 0:
		return specific_reward_cards.duplicate()
		
	# 2. Szukanie pasujących kart w bazie
	var possible_cards = []
	for card_id in CardDatabase.CARDS.keys():
		var card_data = CardDatabase.CARDS[card_id]
		# Zabezpieczenie (Twoja baza ma co najmniej 9 indeksów, rzadkość jest na 8 miejscu)
		if card_data.size() > 8:
			var rarity = card_data[8]
			if rarity in reward_allowed_rarities:
				# Zakładam, że karty powyżej ID 100 to ataki przeciwników (np. "Kęs"), nie chcemy ich losować
				if card_id < 100:
					possible_cards.append(card_id)
					
	# 3. Losowanie z dozwolonej puli
	if possible_cards.size() > 0:
		for i in range(reward_cards_count):
			rewards.append(possible_cards.pick_random())
